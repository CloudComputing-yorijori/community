...
$(document).ready(function () {
  //브라우저 파싱, dom트리 생성전 시작방지 function({})로 축약가능
      ...
      success: function (data) {
        ...
        if (data.ingredients && data.ingredients.length > 0) {
          var ingredientElement = $(
            '<div class="result-item" name="resultIngredien"></div>'
          ).text(data.ingredients[0].ingredientName);
          resultsDiv.append(ingredientElement);
          let hiddenIngredient = $('<input type="hidden" name="ingredi">').val(
            data.ingredients[0].ingredientName
          );
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data.selectMenu[0].menuName == searchMenu) {
            ...
            resultMenu.css({ display: "block", width: "650px" });
