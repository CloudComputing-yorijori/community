  ...
  try {
    ...
    if (postIds.length > 0) {
      ...
      const relatedCategories = [
        ...new Set(relatedPosts.map((post) => post.menu.category)),
      ];
      const relatedTitles = [
        ...new Set(relatedPosts.map((post) => post.title.split(" ")).flat()),
      ];
      ...
      const recommendedPostIds = recommendedPostResults.map(
        (post) => post.postId
      );
      ...
      if (recommendedPostIds.length > 0) {
        const recommendQuery = `
                    ...
                    GROUP BY p.postId;
                `;
        const recommendIngredientResults = await sequelize.query(
          recommendQuery,
          {
        ...
        const recommendIngredientMap = recommendIngredientResults.reduce(
          (map, item) => {
            map[item.postId] = item.ingredients;
            return map;
          },
...
exports.getPostsByCategory = async (req, res) => {
  ...
  try {
    ...
    res.render("recipe/categoryPosts", {
      // 카테고리별 포스트 렌더링할 뷰
      posts: postsWithIngredients,
      category: category,
      showCategoryBar: true,
...
exports.getPostsBySubcategory = async (req, res) => {
  ...
  if (sort === "latest") {
    ...
    order =
      "(SELECT COUNT(*) FROM comments WHERE comments.postId = p.postId) DESC"; // 댓글 많은 순
  }
  ...
  try {
    ...
    res.render("recipe/subCategoryPosts", {
      // 세부 카테고리별 포스트 렌더링할 뷰
      posts: postsWithIngredients,
      category: category,
